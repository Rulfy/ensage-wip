...
function LuaEntityNPC:DamageTaken(dmg,dmgType,source,throughBKB)
	...
	if self:DoesHaveModifier("modifier_undying_flesh_golem_plague_aura") then
		...
		for k,l in pairs(entityList:FindEntities({type = LuaEntity.TYPE_HERO})) do
			if not l.illusion and l.team ~= self.team then
				...
				if spell then
					local baseAmp = .05 * spell.level + .05
		...
		for k,l in pairs(entityList:FindEntities({type = LuaEntity.TYPE_HERO})) do
			if not l.illusion and l.team ~= self.team then					
				...
				if spell then
					...
					tempDmg = tempDmg + threshold*self.maxHealth
					break
				end
			end
		end
	...
	if dmgType == DAMAGE_PHYS then
		...
		if (not throughBKB and self:IsMagicImmune()) or self:IsPhysDmgImmune() or self:IsInvul() then
			tempDmg = 0
		else
	...
	elseif dmgType == DAMAGE_PURE then
		...
		if (not throughBKB and self:IsMagicImmune()) or self:IsInvul() then
			tempDmg = 0
		end
	end
	return tempDmg
...
function LuaEntityNPC:IsRooted()
	return self:IsUnitState(LuaEntityNPC.STATE_ROOTED/2)
end

function LuaEntityNPC:IsDisarmed()
	return self:IsUnitState(LuaEntityNPC.STATE_SOFT_DISARMED/2) or self:IsUnitState(LuaEntityNPC.STATE_DISARMED/2)
end

function LuaEntityNPC:IsAttackImmune()
	return self:IsUnitState(LuaEntityNPC.STATE_ATTACK_IMMUNE/2)
end

function LuaEntityNPC:IsSilenced()
	return self:IsUnitState(LuaEntityNPC.STATE_SILENCED/2)
end

function LuaEntityNPC:IsMuted()
	return self:IsUnitState(LuaEntityNPC.STATE_MUTED/2)
end

function LuaEntityNPC:IsStunned()
	return self:IsUnitState(LuaEntityNPC.STATE_STUNNED/2)
end

function LuaEntityNPC:IsHexed()
	return self:IsUnitState(LuaEntityNPC.STATE_HEXED/2)
end

function LuaEntityNPC:IsInvisible()
	return self:IsUnitState(LuaEntityNPC.STATE_INVISIBLE/2)
end

function LuaEntityNPC:IsInvul()
	return self:IsUnitState(LuaEntityNPC.STATE_INVULNERABLE/2)
end

function LuaEntityNPC:IsMagicImmune()
	return self:IsUnitState(LuaEntityNPC.STATE_MAGIC_IMMUNE/2)
end

function LuaEntityNPC:IsFlying()
	return self:IsUnitState(LuaEntityNPC.STATE_FLYING/2)
end

function LuaEntityNPC:IsUnitWalking()
	return self:IsUnitState(LuaEntityNPC.STATE_NO_COLLISION/2)
end
